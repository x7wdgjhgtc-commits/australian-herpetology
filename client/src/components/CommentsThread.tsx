...
type Hierarchical = AppComment & { replies: AppComment[] };

/**
 * Convert a flat list of comments (already ordered by createdAt asc) into
 * a tree of top-level comments + their replies. Replies-of-replies are
 * flattened to the top-level parent by the backend, so we only have two
 * levels here.
 */
function buildTree(comments: AppComment[]): Hierarchical[] {
  ...
  for (const c of comments) {
    if (c.parentId != null) {
      ...
      else tops.push({ ...c, replies: [] }); // orphaned reply — show as top-level
    }
  }
  return tops;
}
...
function CommentItem({
  ...
  const likeM = useMutation({
    mutationFn: () =>
      comment.likedByMe
        ? apiUnlikeComment(comment.id)
        : apiLikeComment(comment.id),
    onSuccess: () => {
  ...
  const delM = useMutation({
    mutationFn: () => apiDeleteComment(recordId, comment.id),
    onSuccess: () => {
      invalidateKeys.forEach((qk) =>
        queryClient.invalidateQueries({ queryKey: qk }),
      );
        ...
        <div className="rounded-2xl bg-muted/60 px-3 py-2">
          <div className="flex items-baseline gap-2 flex-wrap">
            <Link
              ...
              data-testid={`link-commenter-${comment.id}`}
            >
...
/**
 * Threaded comment section: two-level (top-level + replies).
 * - Per-comment like button
 * - Inline reply input under a top-level comment when "Reply" is clicked
 * - Delete (own / record-owner / moderator)
 *
 * Invalidation: pass a list of queryKeys that should refresh after any
 * mutation (e.g. ["/api/records", recordId], ["/api/feed"], ["/api/records"]).
 */
export function CommentsThread({
  ...
  const addM = useMutation({
    mutationFn: () => apiAddComment(recordId, body.trim()),
    onSuccess: () => {
      setBody("");
      invalidateKeys.forEach((qk) =>
        queryClient.invalidateQueries({ queryKey: qk }),
  ...
  const replyM = useMutation({
    mutationFn: () => {
      ...
      return apiAddComment(recordId, replyBody.trim(), replyParent.id);
    },
    onSuccess: () => {
      setReplyBody("");
      setReplyParent(null);
              ...
              {replyParent?.id === top.id && viewer && (
                <div className="pl-6 sm:pl-9 flex gap-2 items-start">
                  <Textarea
                    ...
                    data-testid={`input-reply-${top.id}`}
                    autoFocus
                  />
                  <div className="flex flex-col gap-1">
                    <Button
                      ...
                      data-testid={`button-submit-reply-${top.id}`}
                    >
                    ...
                    <Button
                      ...
                      data-testid={`button-cancel-reply-${top.id}`}
                    >
                      Cancel
                    </Button>
                  </div>
